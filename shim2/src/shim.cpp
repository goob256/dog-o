...
bool static_start(int sdl_init_flags)
{
	...
	// argc/argv should be set before static_start in shim2 (util::static_start uses them)
	//argc = 0;
	//argv = 0;
	...
	try {
		shim_json = new util::JSON("shim2.json");
	}
	catch (util::Error &e) {
		// Use a default file
		std::string json = "{}";
